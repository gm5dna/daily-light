...
export function loadReadings(): Reading[] {
  ...
  cachedReadings = JSON.parse(raw) as Reading[];
 ...
 */
export function getReading(query: DateQuery, period: Period): Reading | null {
 ...
 */
export function searchReadings(term: string): Array<{ reading: Reading; matches: string[] }> {
  const readings = loadReadings();
  const lower = term.toLowerCase();
  const results: Array<{ reading: Reading; matches: string[] }> = [];
 ...
 */
export function listReadings(includeLeapDay?: boolean): Array<{ date: string; period: Period; themeRef: string; themeText: string }> {
  const readings = loadReadings();
